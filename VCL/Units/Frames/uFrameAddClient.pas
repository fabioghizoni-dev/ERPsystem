...
begin
  ...
  try
    ...
    begin
      ...
      if not ((maskCPF.Text = '   -   -   -  ') and not (maskPhone.Text = '(   )    -    ')) then
      begin

        qry.SQL.Text :=
        'INSERT INTO clientes ' +
        '(id_cliente, nome_cliente, cpf_cliente, celular_cliente, ' +
        'data_adicionado) VALUES (nextval(''clientes_id_cliente_seq'':' +
        ':regclass), '':nome_cliente'', '':cpf_cliente'', '':celular_cliente'',' +
         ''':data_adicionado'');';

        qry.ParamByName('nome_cliente').AsString := nome_cliente;
        qry.ParamByName('cpf_cliente').AsInteger := StrToInt(cpf_cliente);
        qry.ParamByName('celular_cliente').AsString := celular_cliente;
        qry.ParamByName('data_adicionado').AsDate := Now;
...
begin
  ...
  if IsValidCPF(maskCPF.Text) = True then
  begin
